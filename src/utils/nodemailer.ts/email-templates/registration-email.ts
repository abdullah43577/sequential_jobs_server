...
export const registrationEmail = function ({ title, name, message, btnTxt, btnAction }: Props) {
  const date = new Date();

  return mjml2html(`
<mjml>
  <mj-head>
    <mj-title>Sequential Jobs - Job Updates</mj-title>
    <mj-preview>Important update from Sequential Jobs</mj-preview>
  ...
  <mj-body background-color="#f7f9fc">
    ...
    <mj-section background-color="#ffffff" padding="30px 20px" border-radius="8px" css-class="mobile-section">
      <mj-column width="100%">
        ...
        <mj-button background-color="#3498db" color="#ffffff" font-size="16px" font-weight="600" padding="20px 0" border-radius="8px" href="${btnAction}" width="220px" css-class="mobile-button">
          ${btnTxt}
        </mj-button>
      </mj-column>
    </mj-section>
